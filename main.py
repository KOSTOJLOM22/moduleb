...
DEFAULTS = {
    "start_id": 0,
    "target_id": 24,
    # Поле. По умолчанию - конкурсное: сетка 5x5, нумерация по столбцам
    # ("col_major"), как на поле финала ЧВТ. Это основная конфигурация с
    # конкурсного дня, запускается без переопределений через
    # module2_5x5.launch.py (мир factory5x5.wbt, тот же ArucoMap5,
    # что и на реальном поле по раскладке).
    # Старый тренировочный вариант - сетка 6x6, построчная нумерация
    # ("row_major"), топики /RMC2/aruco_id и /RMC2/scan, мир module2.launch.py.
    # Если он ещё нужен - переопредели явно:
    ...
    # Топики РМК-2. По умолчанию - симуляторные: /RMC2/aruco_id и
    # объединённый /RMC2/scan (подходят для module2_5x5.launch.py и старого
    # module2.launch.py - в симуляторе камера и лидар везде одни и те же).
    # На РЕАЛЬНОМ роботе на финальном поле топики другие - отдельная камера
    # нижнего обзора и раздельные лидары, переопредели явно при запуске:
    #   -p aruco_topic:=/RMC2/camera_bottom/aruco_id \
    #   -p scan_topic:=/RMC2/scan_front
    # (на реальном роботе /RMC2/scan - это уже сумма переднего и заднего
    # лидара, в ней виден собственный корпус и задняя полусфера - для
    # навигации не годится).
